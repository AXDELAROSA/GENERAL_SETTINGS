...
--	[PG_CB_EMPLEADO]
--	[PG_CB_IMLOCFIL_SQL]
--	[PG_CB_IMLOCFIL_INVENTARIO_SQL]
--	------------------------------------------------------------------------
--	[PG_CB_CUSTOMER_ARCUSFIL_SQL]
---------	--(0,1) COMBO NORMAL DE CLIENTES	(0=COMBO PARA LAS FICHAS, 1=INCLUYEN EL REGISTRO DE ALL)
---------	--(2) SE USA EN PANTALLA FO_PACKING_NO
---------	--(3) SE USA EN LA FO_COTIZACIONES PARA MOSTRAR LOS CLIENTES INCLUIDOS EN 	ALGUNA COTIZACIÓN
...
---------	--(7) COMBO NORMAL DE CLIENTES CON EL DISPLAY EN INGLES (MUESTRA TODOS LOS CLIENTES)	--SELECT ONE OPTION
---------	--(8) COMBO NORMAL DE CLIENTES CON EL DISPLAY EN INGLES (MUESTRA TODOS LOS CLIENTES)	--ALL
--	------------------------------------------------------------------------
--	[PG_CB_PRODUCT_CATEGORY_IMCATFIL_SQL]
--	[PG_CB_COLOR_IMITMIDX_SQL]
--	[PG_CB_PIEL_CLASIFICACION]
--	[PG_CB_SYSCDEFIL_SQL]
--	[PG_CB_ITEM_INSPECCION_CONFIGURADA]
...
IF EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[PG_CB_EMPLEADO]') AND type in (N'P', N'PC'))
	DROP PROCEDURE [dbo].[PG_CB_EMPLEADO]
GO
-- EXECUTE [PG_CB_EMPLEADO] 001,144, 0

CREATE PROCEDURE [dbo].[PG_CB_EMPLEADO]
	@PP_K_SISTEMA_EXE			INT,
	@PP_K_USUARIO				INT,
	--============================
	@PP_L_CON_TODOS				INT
AS
	...
	DECLARE @VP_TA_CATALOGO	AS TABLE
						(	TA_K_CATALOGO		INT,
							TA_D_CATALOGO		VARCHAR(100),
							TA_O_CATALOGO		INT IDENTITY(1,1)
						)

	INSERT INTO @VP_TA_CATALOGO (TA_K_CATALOGO, TA_D_CATALOGO)
	SELECT	EN_NUM_EMP,
			CONCAT(LTRIM(RTRIM(EP_NOMBRE)), ' ', LTRIM(RTRIM(EP_APELLIDO_PATERNO)), '     ', LTRIM(RTRIM(DP_DESC_DEPTO)),'     (', EN_NUM_EMP, ')') 
...
AS
	...
	--		ORDER BY TA_D_CATALOGO
		...
		--END
		-- ====================================================================================
		--SELECT * FROM [DATA_02].[dbo].imlocfil_sql	(NOLOCK)
		-- ====================================================================================
...
AS
	...
	BEGIN
		...
		INSERT INTO @VP_TA_CATALOGO 
		SELECT	K_ARCUSFIL	,
				ARCUSFIL_PROGRAM.CUS_NO			,
				1,--ARCUSFIL_SQL.O_ARCUSFIL		,
				ARCUSFIL_SQL.L_BORRADO		,
				ARCUSFIL_SQL.L_ARCUSFIL		
		FROM	[DATA_02].[dbo].ARCUSFIL_PROGRAM	(NOLOCK)
		INNER JOIN	[DATA_02].[dbo].ARCUSFIL_SQL		(NOLOCK)	ON ARCUSFIL_SQL.A4GLIDENTITY	= ARCUSFIL_PROGRAM.K_ARCUSFIL
		WHERE	ARCUSFIL_PROGRAM.CUS_NO		NOT IN (	SELECT	TA_D_CATALOGO
														FROM	@VP_TA_CATALOGO	)
	...
	BEGIN
			...
			WHERE	ARCUSFIL_PROGRAM.L_BORRADO		= 0
...
-- EXECUTE [dbo].[PG_CB_COLOR_IMITMIDX_SQL] 1, 144, 5
-- USE [BD_GENERAL]
...
AS
	...
	-- SE AGREGÓ COMO COMBO PARA MOSTRAR LOS COLORES EN LA PANTALLA DE COLORES NO CONTROLADOS. SE CARGABA ANTES CON LA CONSULTA: ( select color from colors )		--	AX: 20211122	
	IF @PP_L_CON_TODOS=5
	BEGIN
		INSERT INTO @VP_TA_CATALOGO 
		SELECT	DISTINCT
...
--		 EXECUTE [dbo].[PG_CB_FOB_POINT] 0,0,2
...
--		 EXECUTE [dbo].[PG_CB_FREIGHT_CHARGES] 0,0,2
...
--		 EXECUTE [dbo].[PG_CB_TAXABLE] 0,0,2
...
--		 EXECUTE [dbo].[PG_CB_ARCUSFIL_TERMS_PERIOD] 0,139,2
...
--		 EXECUTE [dbo].[PG_CB_TERMS] 0,139,2
