...
--	IF @VP_MENSAJE<>''
	...
	SELECT	TOP (5000)
			...
			APELLIDO_PATERNO_FORANEO AS APELLIDO_PATERNO,
			APELLIDO_MATERNO_FORANEO AS APELLIDO_MATERNO,
			NOMBRE_FORANEO AS NOMBRE,
...
IF EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[PG_LI_GRUPO_TAG]') AND type in (N'P', N'PC'))
	DROP PROCEDURE [dbo].[PG_LI_GRUPO_TAG]
GO

-- EXECUTE [dbo].[PG_LI_GRUPO_TAG] 0,139,139
CREATE PROCEDURE [dbo].[PG_LI_GRUPO_TAG]
	@PP_K_SISTEMA_EXE			INT,
	@PP_K_USUARIO_ACCION		INT,
	-- ===========================
...
AS
	-- =========================================
	SELECT		GRUPO_TAG.K_GRUPO_TAG,
				D_GRUPO_TAG
	FROM		GRUPO_TAG 
	WHERE		K_GRUPO_TAG<>0
	ORDER BY	K_GRUPO_TAG
	
--	SELECT DISTINCT		
--				SISTEMA_TAG.K_GRUPO_TAG,
--				D_GRUPO_TAG
--	FROM		USUARIO_PEARL
--	INNER JOIN	USUARIO_PERMISOS ON USUARIO_PEARL.K_USUARIO_PEARL =USUARIO_PERMISOS.K_USUARIO_PEARL
--	LEFT JOIN	SISTEMA_TAG ON USUARIO_PERMISOS.K_SISTEMA_TAG=SISTEMA_TAG.K_SISTEMA_TAG
--	LEFT JOIN	GRUPO_TAG ON SISTEMA_TAG.K_GRUPO_TAG=GRUPO_TAG.K_GRUPO_TAG
...
IF EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[PG_LI_SISTEMA_TAG]') AND type in (N'P', N'PC'))
	DROP PROCEDURE [dbo].[PG_LI_SISTEMA_TAG]
GO

-- EXECUTE [dbo].[PG_LI_SISTEMA_TAG] 0,139,139,30
CREATE PROCEDURE [dbo].[PG_LI_SISTEMA_TAG]
	@PP_K_SISTEMA_EXE				INT,
	@PP_K_USUARIO_ACCION			INT,
	-- ===========================
	@PP_K_USUARIO_PEARL				INT,
...
AS
	...
	SELECT DISTINCT		SISTEMA_TAG.K_GRUPO_TAG,
						...
						D_SISTEMA_TAG
	FROM				SISTEMA_TAG
	WHERE		SISTEMA_TAG.K_GRUPO_TAG=@PP_K_GRUPO_TAG
	AND			K_GRUPO_TAG<>0
	AND			K_SISTEMA_TAG<>0
	ORDER BY	K_GRUPO_TAG,K_SISTEMA_TAG
...
-- /////////////////////////////////////////////////////////////////////
	...
	--============================================================================
	-----=====================================================
	-- SE HACE UN CURSOR PARA OBTENER CADA UNO DE LOS COLORES INSERTADOS EN EL GRID DE COLORES
